...
listenerMiddleware.startListening({
    ...
    effect: async (action,listenerApi) => {
        if (action?.payload?.error) {   //showPopupMessageAction的参数是传给异步函数(message: Message) => {}的参数
...
export const store = configureStore({
    ...
    middleware: (getDefaultMiddleware) =>
        getDefaultMiddleware()
            .concat(rehybApi.middleware)    //.concat(api.middleware): 这个方法会将你的自定义中间件添加到默认中间件数组的末尾。这意味着你的自定义中间件将在默认中间件之后执行。
