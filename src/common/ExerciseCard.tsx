...
export const ExcersiceSamllCard: FC<{
    style?: CSSProperties, exercise: Exercise
}> = ({ exercise }) =>
    {
        const likeRateColor = exercise.LikeRate !== undefined
        ...
        return (
            <div className="card flex items-center gap-6 min-w-min max-w-[200px] cursor-grab relative" >
                {/* Left Section */}
                <div className="flex-col flex items-start gap-4  min-w-[200px]">
                    <div className="flex items-center gap-2">
                        <IconInCircle icon={<GiBiceps />} size={6} />
                        <h3>{exercise.ProtocolName ?? ''}</h3>
                    </div>
                ...
                {exercise.LikeRate !== undefined && (
                    <div className={`flex items-center gap-1 self-end`}>
                        <div className={`text-${likeRateColor} flex`}>
                            {exercise.LikeRate >= 0 ? (
                                <AiFillLike className={`w-6 h-6 fill-${likeRateColor}`} />
                            ) : (
                                <AiFillDislike className={`w-6 h-6 fill-${likeRateColor}`} />
                            )}
                            <span>{Math.round(exercise.LikeRate * 100)}%</span>
                        </div>
                    </div>
                )}
            </div>
